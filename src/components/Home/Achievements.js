      ...
      <div className="w-[80%] h-full text-left" ref={sectionRef}>
        ...
        <div className="grid grid-cols-2 gap-[1vw] max-[450px]:mt-[6vw]">
          ...
          {Content.boxes.slice(0, 2).map((box, index) => (
            <div
              key={index}
              className="bg-[#2B00AC] text-white h-[32vw] text-left items-center pt-[5vw] pl-[3vw] rounded-md"
            >
              <h1 className="text-[5vw] font-extrabold max-[450px]:text-[8vw] max-[450px]:mt-[-5vw]">
                {sectionInView ? (
                  <CountUp start={0} end={box.count} duration={3} />
                ) : (
                  0
                )}
              </h1>
        ...
        <div className="col-span-3 grid grid-cols-3 gap-[1vw] mt-[1vw]">
          {Content.boxes.slice(2).map((box, index) => (
            <div
              key={index}
              className="border-[0.1vw] border-[#2B00AC] h-[22vw] text-left pt-[3vw] pl-[2vw] space-y-[1vw] rounded-md hover:bg-[#2B00AC] hover:pt-[4vw] transition-all ease-in-out duration-300 hover:text-white"
            >
